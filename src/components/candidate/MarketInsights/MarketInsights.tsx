...
export function MarketInsights({ isCollapsed }: MarketInsightsProps) {
  return (
    <div className="space-y-4 min-w-[280px]">
      <AnimatePresence>
        {!isCollapsed && (
          <motion.h3
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="text-sm font-bold uppercase tracking-wider text-zinc-500 opacity-60 px-4"
          >
            Market Insights
          </motion.h3>
        )}
      ...
      <div className="bg-surface-low p-4 rounded-2xl space-y-4">
        {/* Insight 1 */}
          ...
          <AnimatePresence>
            {!isCollapsed && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -10 }}
              >
                <h4 className="text-sm font-bold text-zinc-900 group-hover:text-brand transition-colors leading-snug whitespace-normal">
                  The rise of AI-Orchestrated hiring in 2024.
                </h4>
                <p className="text-[11px] text-zinc-500 mt-1">5 min read</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          <AnimatePresence>
            {!isCollapsed && (
              ...
              >
                ...
                <p className="text-[11px] text-zinc-500 mt-1">8 min read</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
